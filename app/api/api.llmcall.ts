...
// ★ Cloudflare context を使わない Node 版アクション
...
async function llmCallAction({ request }: ActionFunctionArgs) {
  ...
  if (!model || typeof model !== 'string') {
    return new Response('Invalid or missing model', { status: 400 });
  }
  ...
  // ★ Nodeランタイムでは context.env は存在しない → process.env
  const serverEnv = process.env as any;
